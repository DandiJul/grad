...
try {
  ...
  console.log('-> accounts.json file not found, using environment variables')
...
if (accounts.length === 0) {
  console.error("No accounts configured! Please add accounts to accounts.json or env")
  process.exit(1)
}
...
async function downloadExtension(extensionId) {
  ...
  if (fs.existsSync(EXTENSION_FILENAME)) {
    console.log("-> Extension already downloaded! Skipping download...")
    return
  }
...
async function runAccount(account) {
  ...
  try {
    ...
    console.log(`-> ${account.user} logged in! Opening extension...`)
